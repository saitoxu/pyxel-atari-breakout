...
class App:
    def __init__(self):
        ...
        self.ball = Circle(Config.WIDTH / 2, Config.HEIGHT / 2, 4, color=7)
        self.lines = [
        ...
        _len = 32
        self.bricks = [
            Rectangle(0, 0, _len, 8, color=10),
            Rectangle(  _len, 8  , _len, 8, color=9),
            Rectangle(2*_len, 8*2, _len, 8, color=8),
            Rectangle(3*_len, 8*3, _len, 8, color=10),
            Rectangle(4*_len, 8*4, _len, 8, color=9),
            Rectangle(5*_len, 8*5, _len, 8, color=8),
            Rectangle(6*_len, 8*6, _len, 8, color=10),
            Rectangle(7*_len, 8*7, _len, 8, color=9),
            # Rectangle(
            #     Config.WIDTH - _len - 8,
            #     8,
            #     _len,
